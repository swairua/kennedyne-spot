...
import { Save, Eye, Send, ArrowLeft, Clock, Star, Tag, User, FileText, Search, X, Upload, Settings, Pencil, Globe, Loader2, Maximize2 } from 'lucide-react';
...
import { EditorViewModes, type ViewMode } from '@/components/editor/EditorViewModes';
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      {previewMode ? (
        ...
        <Tabs defaultValue="write" className="space-y-6">
          ...
          {/* WRITE TAB */}
          <TabsContent value="write" className="space-y-6 pb-24">
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="min-h-[65vh]">
                  {viewMode === 'write' && (
                    <RichTextEditor
                      ...
                      onChange={(markdown) => { setPost(prev => ({ ...prev, content: markdown })); setHasUnsavedChanges(true); }}
                  ...
                  {viewMode === 'split' && (
                    <ResizablePanelGroup direction="horizontal" className="min-h-[65vh] rounded-lg border">
                      ...
                      <ResizablePanel defaultSize={45} minSize={25}>
                        <div className="h-full overflow-auto bg-muted/20 p-6">
                          <div className="prose prose-sm dark:prose-invert max-w-none">
                            ...
                            {post.excerpt && (
                              <p className="text-muted-foreground text-sm italic mb-4">{post.excerpt}</p>
                            )}
                            {post.content ? (
                              <EnhancedMarkdownRenderer
                                content={post.content}
                  ...
                  {viewMode === 'preview' && (
                    <div className="min-h-[65vh] rounded-lg border bg-muted/20 p-8 overflow-auto">
                      <div className="prose prose-lg dark:prose-invert max-w-3xl mx-auto">
                        {post.title && (
                          <h1 className="text-3xl font-bold mb-4">{post.title}</h1>
                        )}
                        {post.excerpt && (
                          <p className="lead text-muted-foreground italic mb-6">{post.excerpt}</p>
                        )}
                ...
                <p className="text-xs text-muted-foreground">
                  Use the view mode switcher above to toggle between Write, Split, and Preview modes. Click Focus for distraction-free writing.
                </p>
              </CardContent>
            </Card>
          </TabsContent>
      ...
      <FocusMode
        ...
        content={post.content}
        onChange={(content) => { setPost(prev => ({ ...prev, content })); setHasUnsavedChanges(true); }}
        title={post.title}
      />
      ...
      <FloatingActionBar
        wordCount={wordCount}
        readingTime={post.reading_time_mins}
        hasUnsavedChanges={hasUnsavedChanges}
        saving={saving}
        onSave={() => { handleSave(); setHasUnsavedChanges(false); }}
        onPreview={() => setViewMode('preview')}
        onPublish={() => { handleSave(true); setHasUnsavedChanges(false); }}
      />
    </div>
  );
}
