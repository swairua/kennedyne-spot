...
import { InlineCTABlock, renderCTAFromMarkdown } from './InlineCTABlock';
...
export const EnhancedMarkdownRenderer: React.FC<EnhancedMarkdownRendererProps> = ({
  ...
  const components = {
    ...
    // Enhanced paragraphs with CTA block detection
    p: ({ children, ...props }: any) => {
      ...
      // Check for inline CTA syntax: [CTA:type:title:buttonText:url]
      const text = typeof children === 'string' ? children : '';
      const ctaMatch = text.match(/\[CTA:(\w+):([^:]+):([^:]+):([^\]]+)\]/);
      if (ctaMatch) {
        return renderCTAFromMarkdown(text);
      }
    ...
    // Enhanced code blocks with error handling
    pre: ({ children, ...props }: any) => {
      try {
        ...
        return <pre className="bg-muted p-4 rounded-lg overflow-x-auto" {...props}>{children}</pre>;
      }
    },
    ...
    // Enhanced images - handle float styles and links
    img: ({ src, alt, title, ...props }: any) => {
      // Check if parent is an anchor (linked image)
      const hasLink = props.parentNode?.tagName === 'a' || props['data-linked'];
      
      // For images with explicit sizing/alignment (from HTML), render as-is
      if (props.className || props.width || props.height || props.style) {
        const imgElement = (
          <img 
            src={src} 
            alt={alt || ''} 
      ...
      // For markdown images, use Figure component
      return (
        <Figure 
          src={src} 
          alt={alt || ''} 
    ...
    // Handle figure elements with float styles
    figure: ({ children, style, className, ...props }: any) => {
      // Parse style string to detect wrap mode
      let wrapMode: 'inline' | 'float-left' | 'center' | 'float-right' | 'full' = 'center';
      const styleStr = typeof style === 'string' ? style : '';
      
      if (styleStr.includes('float: left') || className?.includes('float-left')) {
        wrapMode = 'float-left';
      } else if (styleStr.includes('float: right') || className?.includes('float-right')) {
        wrapMode = 'float-right';
      } else if (className?.includes('full-width') || styleStr.includes('width: 100%')) {
        wrapMode = 'full';
      } else if (className?.includes('inline') || styleStr.includes('inline-block')) {
        wrapMode = 'inline';
      }

      const figureClasses = cn(
        className,
        wrapMode === 'float-left' && 'image-float-left',
        wrapMode === 'float-right' && 'image-float-right',
        wrapMode === 'full' && 'image-full-width',
        wrapMode === 'center' && 'image-center',
        wrapMode === 'inline' && 'image-inline'
      );

      return (
        <figure className={figureClasses} style={style} {...props}>
          {children}
        </figure>
      );
    },

    // Handle anchor tags - detect if wrapping an image
    a: ({ href, children, ...props }: any) => {
      const isInternal = href?.startsWith('/') || href?.startsWith('#');
      
      // Check if this anchor wraps an image
      const hasImageChild = React.Children.toArray(children).some(
        (child) => React.isValidElement(child) && (child.type === 'img' || (child.props as any)?.src)
      );

      if (hasImageChild) {
        // This is an image link - render with special styling
        return (
          <a
            href={href}
